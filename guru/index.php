<!DOCTYPE html>
<html>
<head>
	<title>Table Teacher</title>
  <link rel="stylesheet" type="text/css" href="../css/style.css">
	
</head>
<body>

<header>

	<?php include("../header.php"); ?>

</header>

<!--
    Zero Project - DanZai Web. By Danis Zaidan
    (Dilarang menjiplak/mengedit/mengakui tanpa seijin pembuat)
    Script HTML By DANis ZAIdan
    Script PHP By DANis ZAIdan
    Style CSS By DANis ZAIdan
    Copyright 2016
-->

<div class="kabeh">  
  <div class="judul"><h3>Table Teacher</h3></div>
  <div class="isi">
    <p class="note">View Table : Guru</p>
    <div class="ket"><p>Data diri para Guru di SMK Al-Irsyad Tegal</p></div>
		<table width="auto" cellspacing="2" class="tabel">
			<tr class="warna">
				<td>No</td>
				<td>NIPY</td>
				<td>Nama</td>
				<td>Alamat</td>
				<td>Prodi</td>
				<td>No Telpon</td>
				<td>Action</td>
			</tr>

			<?php

				include("../lib/koneksi.php");

				$nomor=1;
				$hasil = mysql_query("select * from tb_guru");
				while($cetak=mysql_fetch_row($hasil)){	
					$nipy		= $cetak[0];
					$nama 		= $cetak[1];
					$alamat 	= $cetak[2];
					$prodi		= $cetak[3];
					$no_telp	= $cetak[4];
			?>

			<tr>
				<td class="warna"> <?php echo $nomor; ?> </td>
				<td class="warna2"> <?php echo $nipy ?> </td>
				<td class="warna2"> <?php echo $nama ?> </td>
				<td class="warna2"> <?php echo $alamat ?> </td>
				<td class="warna2"> <?php echo $prodi ?> </td>
				<td class="warna2"> <?php echo $no_telp ?> </td>
				<td class="act"> 
					<center><a class="action1" href="editdata.php?id=<?php echo $nipy ?>" > Edit </a>
					<a class="action2" href="hapusdata.php?id=<?php echo $nipy ?>" > Hapus </a></center>
				</td>

			<?php 
				$nomor++;
				} 
			?>
			</tr>
		</table>
	</div>

	<footer class="footer">
  		<p>Copyright &copy <a href="http://nexiapedia.blogspot.com/">DanZai</a> 2016</p>
	</footer>

</div>

</body>
</html>